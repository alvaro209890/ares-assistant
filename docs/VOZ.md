...
- **Heartbeat** (`startHeartbeat`): tarefas com mais de 15 s geram atualização exibida e falada a cada 30 s. O rótulo é dinâmico e tolera paralelismo: quando várias ferramentas rodam ao mesmo tempo, a fala usa a frente ativa mais recente e volta para a remanescente se a mais nova terminar.
...
- **Aterramento de fala** (`groundCodeSpeech`): antes de gravar a resposta final, o Ares confere se há evidência real para afirmações como "alterei", "rodei" ou "validei". Se não houver, a fala vira bloqueio honesto; se houver autorização pendente ou falha de validação, isso prevalece sobre qualquer conclusão otimista.
- **Promessa sem ação**: se o modelo diz que vai alterar/rodar/chamar algo de programação mas devolve `acoes: []`, o orquestrador faz uma rodada corretiva interna exigindo ações `codigo.*`/`subagente.*` ou um bloqueio explícito.
...
`tests/tts.test.ts`, `tests/speech.test.ts`, `tests/voice-code.test.ts`, `tests/voice-control.test.ts`, `tests/piper-engine.test.ts`, `tests/subagents.test.ts`, `tests/coder.test.ts` e `tests/agent.test.ts` cobrem fila, normalização, anúncios, heartbeat dinâmico/paralelo, aterramento de fala, limiares, pool do Piper, parsing de relatórios, coder autônomo e o contrato de transcript único em voz+programação. Gate: `npm run verify`.
