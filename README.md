...
- **Destaques da v0.33.0 (Colmeia: Manager + subagentes especialistas)**:
  - **Arquitetura Manager/Worker**: o Ares agora orquestra uma equipe de subagentes — **Investigador** (pesquisa web/documentacao e devolve so fatos com fonte), **Construtor** (projeta a implementacao: arquivos, codigo pronto e ordem de aplicacao) e **Critico** (roda o diagnostico do projeto e emite parecer rigoroso com veredito). Todos usam o MESMO provedor/modelo do Ares (`ninerouter`), variando apenas o system prompt e a temperatura por especialidade (`src/main/subagents/`). Os subagentes nunca falam: devolvem relatorio tecnico e o Ares sintetiza por voz/texto.
  - **Novas ferramentas**: `subagente.pesquisar {objetivo, consulta?, url?}`, `subagente.construir {objetivo, path?}` e `subagente.auditar {objetivo, path?}` — o Ares delega sozinho em tarefas grandes ("pesquise a fundo", "projete a mudanca", "audite o projeto") e responde direto nas simples.
  - **Aba "Escritorio" (Alt+9)**: dashboard da Colmeia com o Ares (Manager) no topo, cards animados dos tres especialistas (ocioso/trabalhando/relatorio/falha) e linhas de conexao com fluxo animado quando a informacao circula; chat unificado com texto e microfone igual a aba principal. Status em tempo real via canal IPC `agent:hive-update`.
